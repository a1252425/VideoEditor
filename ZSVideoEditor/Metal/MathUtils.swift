...
extension matrix_float4x4 {
  var xy_2d: matrix_float2x2 {
    matrix_float2x2(vector_float2(self[0][0], self[0][1]),
                    vector_float2(self[1][0], self[1][1]))
  }
  
  static var identity: matrix_float4x4 {
    matrix_float4x4(vector_float4(1, 0, 0, 0),
                    vector_float4(0, 1, 0, 0),
                    vector_float4(0, 0, 1, 0),
                    vector_float4(0, 0, 0, 1))
  }
  
  static func translation(_ position: vector_float3) -> Self {
    matrix_float4x4(vector_float4(1, 0, 0, 0),
                    vector_float4(0, 1, 0, 0),
                    vector_float4(0, 0, 1, 0),
                    vector_float4(position.x, position.y, position.z, 1))
  }
  
  static func scale(_ scale: Float) -> matrix_float4x4 {
    return matrix_float4x4(vector_float4(scale, 0, 0, 0),
                           vector_float4(0, scale, 0, 0),
                           vector_float4(0, 0, scale, 0),
                           vector_float4(0, 0, 0, 1))
  }

  static func rotate(_ angle: Float, axis: vector_float3) -> matrix_float4x4 {
  ...
  static func model() -> matrix_float4x4 {
    let scaled = scale(0.5)
    let rotatedY = rotate(.pi / 4, axis: vector_float3(0, 1, 0))
    let rotatedX = rotate(.pi / 4, axis: vector_float3(1, 0, 0))
    return matrix_multiply(matrix_multiply(rotatedX, rotatedY), scaled)
  }

  static func view() -> matrix_float4x4 {
    let cameraPosition = vector_float3(0, 0, -3)
    return translation(cameraPosition)
  }

  static func projection(near: Float, far: Float, aspect: Float, fovy: Float) -> matrix_float4x4 {
